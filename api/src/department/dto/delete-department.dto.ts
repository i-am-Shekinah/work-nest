...
export class DeleteDepartmentDto {
  ...
  @IsEnum(DepartmentDeleteAction, { message: `action must be one of: ${Object.values(DepartmentDeleteAction).join(', ')}` })
  action: DepartmentDeleteAction;


  @ApiPropertyOptional({
    description: 'ID of the department to reassign employees to (required if action is REASSIGN)',
